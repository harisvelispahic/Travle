...
**Napomena o pretrazi s više odabranih kategorija:** filter kategorija na pretrazi je višestruki odabir. Svaka odabrana kategorija je izraženi interes korisnika, pa se **za svaku odabranu kategoriju zapisuje po jedan `Search` red pune težine** (isto kao što onboarding zapisuje po jedan red za svaki odabrani interes). Zbog normalizacije profila (§4.4) veći broj odabranih kategorija ne čini profil „jačim“ — samo raspoređuje interes ravnomjerno između odabranih kategorija. Ako korisnik nije odabrao nijednu kategoriju, termin pretrage se pokušava poklopiti s nazivom kategorije, a zatim s nazivom taga; ako se ne poklopi ni s čim, red se svejedno zapisuje sa `SearchTerm`-om (bez osobine, pa ne utiče na profil). Interakcija se bilježi samo kada pretraga sadrži tekst — sam odabir filtera bez teksta se ne bilježi, jer se pretraga izvršava i pri samom otvaranju ekrana.
