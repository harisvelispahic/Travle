...
{
    /// <summary>
    /// Shape validation for a new tour. That the tour type exists and every destination id references an
    /// approved destination are business rules verified in the service (they need the database).
    /// </summary>
    public class TourInsertValidator : AbstractValidator<TourInsertRequest>
    {
        public TourInsertValidator()
        {
            RuleFor(x => x.Name)
