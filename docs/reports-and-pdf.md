...
Desktop (ReportsScreen)
  │  user picks a period, clicks "Download PDF" or "Print"
  ▼
...
Desktop: saveReportPdf(bytes, name)              UI/travle_desktop/lib/util/report_download.dart
     FilePicker.saveFile → write bytes → snackbar with "Open" (OS default viewer)
  or printReportPdf(bytes, documentName)          same file
     Printing.layoutPdf → OS print dialog (nothing written to disk)
```

Both actions call the same endpoint and print/save the same server-generated bytes; they differ only in
...
Each report has **two** actions, side by side:

- **Download PDF** — `saveReportPdf` opens the native Save dialog via `FilePicker.saveFile`, writes the
  bytes, and shows a snackbar with an **Open** action that launches the file in the OS default viewer
  (`cmd /c start` on Windows).
- **Print** — `printReportPdf` hands the same bytes to `Printing.layoutPdf`, which opens the platform's
  own print dialog. Nothing is written to disk on this path.

> **Changed 2026-09-09 (August review, finding 8).** This previously argued that a generated PDF is
> inherently printable, so no in-app print dialog and no `printing` dependency were needed — the Open
> action put the user one Ctrl+P from a printout. The review rejected that reading: RS2 asks for Download
> *and* Print as two actions **in the application**, and delegating the second to an external viewer does
> not satisfy it. The `printing` package (5.14.x, desktop-capable on Windows/macOS/Linux) is now a
> dependency of `travle_desktop`, and each report renders a Print button beside Download with its own
> busy flag.
>
> The backend is untouched: same QuestPDF documents, same endpoints, same bytes. Adding the plugin also
> regenerated the Linux and macOS plugin registrants, which is Flutter's normal behaviour for a new
> dependency.
