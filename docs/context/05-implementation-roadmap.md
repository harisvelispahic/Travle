...
## Later / optional (only if time, see 00 §3): "near you" (needs a runtime geolocation permission — **dropped** by user); onboarding category-card polish (needs `Category.Description`/`Image` + a migration); Stripe CLI compose profile (dev convenience). *(The old "geocoding search in the map picker" item is superseded by Phase S5.)*
